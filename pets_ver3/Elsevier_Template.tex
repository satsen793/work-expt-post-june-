...
We report \emph{reward-aligned performance} (cumulative reward and variance) together with \emph{educational outcomes} (time-to-mastery, post-content gain, blueprint adherence) for the PPO controller. Cumulative reward includes penalties for blueprint deviation and frustration, so mastery speed and reward can diverge; we therefore pair the reward summary with modality gains and calibration for interpretability.

\subsection{Training dynamics and end-of-training performance summary}
\label{subsec:training_summary}
Figure~\ref{fig:learning_curve} shows PPO learning dynamics (batch mean reward across seeds). We summarize end-of-training metrics in Table~\ref{tab:perf_summary}, generated directly from the experimental outputs.
